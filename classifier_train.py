...
def gradient_descent(W, b, n_iters = 1000, batch_size = 2**8, learning_rate = .002, beta1 = .9, beta2 = .999, decay_rate = 1.):
    VdW, Vdb = init_adam()
    SdW, Sdb = init_adam()
    for iter_idx in range(n_iters):
        ...
        for batch_idx in range(n_batches):
            ...
            cost = cross_entropy_loss(A[L - 1], Y_cur)
            #cost = train_set_performance()
            epoch_num = iter_idx * n_batches + batch_idx + 1
            print("Cost after " + str(epoch_num) + " iterations: " + str(cost) + '.')
            dW, db = backward_prop(X_cur, Y_cur, W, b, Z, A)
...
gradient_descent(W, b, learning_rate=.002)
